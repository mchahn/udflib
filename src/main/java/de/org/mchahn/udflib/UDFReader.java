...
import de.org.mchahn.baselib.io.BlockDeviceReader.Exception.Code;
...
public class UDFReader extends BlockDeviceReader {
    static final Log _log = new Log("udf.reader");
    ...
    private final void readBlock(long num) throws IOException {
        readBlock(num, this.block);
    }
    private final byte[] readBlockNew(long num) throws IOException {
        byte[] result = new byte[this.blockSize];
        readBlock(num, result);
        return result;
    }
    private final void readBlock(long num, byte[] buf) throws IOException {
        try {
    ...
    private final byte[] readLogicalBlockNew(int num) throws IOException {
        byte[] result = new byte[this.blockSize];
        readLogicalBlock(num, result);
        return result;
    }
    private final void readLogicalBlock(int num, byte[] buf) throws IOException {
        readBlock(blockAddressToLogicalBlock(num), buf);
    }
    private final long blockAddressToLogicalBlock(int num) {
